...
class AVLTreeMap(TreeMap):
  ...
  def _isbalanced(self, p):
    ...
    #print(p._node._balance_factor)
    return p._node._balance_factor <= 1 and p._node._balance_factor >= -1

  def _tall_child(self, p, favorleft=False): # parameter controls tiebreak
    #if (p._node._balance_factor > 0) or (p._node._balance_factor == 0 and favorleft):
    if (p._node._balance_factor > 0):
      return self.left(p)
    else: #if p._node._balance_factor < 0:
      return self.right(p)
    '''
  ...
  def _rebalance_insert(self, p):
    ...
    while p is not None:  
      ...
      if not self._isbalanced(p):
        cont = False
        temp = p._node._balance_factor
        #print("grandchild:",self._tall_grandchild(p).element()._value, p.element()._key)
        p = self._restructure(self._tall_grandchild(p))
        #print("val:",p.element()._value)
        ...
        if self.parent(p) and p == self.left(self.parent(p)):
          self.parent(p)._node._balance_factor += 1
        elif self.parent(p):
          self.parent(p)._node._balance_factor -= 1
...
print()
'''
print("Root:",T.root().element()._value)
if T.left(T.root()):
  print("Left:",T.left(T.root()).element()._value)
if T.right(T.root()):
  print("Right:",T.right(T.root()).element()._value)
'''
print()
print("Elements:", len(T))
print("Height of Tree Calculated at End:", T.height()) 
print("BALANCED?", T._isbalanced(T.root()))
print("Balance Factor of Root:", T.root()._node._balance_factor)

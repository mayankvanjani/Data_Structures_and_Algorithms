...
class Tree:
    ...
    def flip_subtree(self,p=None):
        ...
        self.flip(p)
        if self.left(p):
            self.flip_subtree(self.left(p))
        if self.right(p):
            self.flip_subtree(self.right(p))
    
    def bring_left_up(self,p):
        root = self.left(p)
        right = p
        right._node._left = right._node._left._right
        root._node._right = right._node
        if self.root() == p:
            self._root = root._node
        else:
            self._root._left = root._node
...
def fractionTree(n, start = "1/2"):
    T = Tree()
    if n > 0:
        slash = start.find("/")
        left = int(start[:slash]) - 1 + int(start[:slash])
        right = left + 2
        start_left = str(left) + "/" + str( int(start[slash+1:]) * 2)
        start_right = str(right) + "/" + str( int(start[slash+1:]) * 2)
        T.attach(T.add_root(start), fractionTree(n-1,start_left), fractionTree(n-1,start_right))
    return T
